...
app.listen(PORT, async () => {
    ...
    console.log(`≡ƒÜù Driver Pulse Server ready on :${PORT}`)
    console.log(`≡ƒôè Available trips (motion): ${motionTrips.join(', ')}`)
    console.log(`≡ƒÄ╡ Available trips (audio):  ${audioTrips.join(', ')}`)
    console.log(`ΓÜí Fusion endpoint: /api/fusion/:tripId`)
})
